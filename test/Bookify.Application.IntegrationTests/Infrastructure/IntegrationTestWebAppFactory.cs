...
public class IntegrationTestWebAppFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        builder.ConfigureTestServices(services =>
        {
            ...
            services.RemoveAll<IJobFactory>();
            services.AddSingleton<ISchedulerFactory>(provider =>
            {


                // Construct a unique scheduler name using a base name and a unique identifier
                // string schedulerName = $"TestScheduler_{DateTime.Now:yyyyMMddHHmmsss}";
                string schedulerName = $"TestScheduler_{DateTime.Now.Ticks.ToString()}";

                // Customize scheduler properties as needed
                var properties = new NameValueCollection
                {
                    { "quartz.scheduler.instanceName", schedulerName },
                };

                // Create and return an instance of the scheduler factory
